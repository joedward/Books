...
* displacement, delta, scalar, vector, speed, velocity, acceleration, average, instantaneous
...
Displacement is the change in position of an object. When working in one dimension the direction of change is indicated by a + or - in front of the size of the displacement. 
...
The $$\Delta$$ \(Greek Delta\) symbol tells us to subtract two numbers. We only need the final and initial position values. We do not need to keep track of the intermediate positions of the object.
...
The measure of the rate of change of velocity is called acceleration. The average acceleration is found by dividing the \(change in velocity\) by the \(change in the time\).

$$
a_{avg}=\frac{\Delta v}{\Delta t}
$$

The direction of the acceleration should be specified, but this can be done with + and - .
...
Question: TRUE or FALSE? If the velocity is zero then the acceleration is zero.

Question: TRUE or FALSE? If the velocity is positive then the acceleration is positive.

Question: TRUE or FALSE? If the acceleration is zero then the velocity is zero.

Question: What is the meaning of an acceleration of - 3 m/s2?

You can find a list of instructional, pre-class, YouTube videos for introductory physics lectures, and videos of example problems, at [http://physics.gpclements.com](http://physics.gpclements.com).

### Section 2.5 Motion Equations for Constant Acceleration in One Dimension
...
1. By definition a = V / t or a = \(V - V0\) / t. Multiply both sides by t and add V0 to both sides. **Equation 1 V = V0 + at2.** You should understand how equation 2.28 is found . Equation 2.28 lets us calculate the final position if we know the initial position, the average velocity and the time. It turns out that the average velocity can be found by adding the initial and final velocities and then dividing by 2.

**X = X0 + ½ \(V + V0\) t.** This is only valid when the acceleration is constant. Why?

I will refer to this as **Equation 2**.

Suppose the acceleration is 4 m/s2, the initial velocity is +3 m/s, and the initial position is 7 meters. a\) What is the velocity when t = 5 seconds?

Answer:

**V = V0 + at**

V = 3 m/s + 4 m/s2 \* 5 sec

V = 23 m/s

b\) What is the position of the object at t = 5 seconds?
...
1. near page 57 in pdf We can combine the first two equations by substituting for V in equation 2 with the expression for V from equation 1. This will generate an equation that does not contain the final velocity.
2. Start with equation 2 X = X0 + ½ \(V + V0\) \* t

Replace V with the expression from equation 1: X = X0 ½ \( \[V0 at\] + V0\) \* t

You should use the principles of algebra to simplify this. Tell me if you don't get

**Equation 3 X = X0 + V0 \* t + 0.5 \* a \* t**2
...
It is worth examining equation 3 for the case of acceleration = 0. With a = 0 the equation becomes

X = X0 V0 \* t or \(X - X0\) = V0 \* t

You may recognize this as being similar to your math problems from high school …. Distance = rate \* time. D = rt D = rt is only valid if the rate is constant \(i.e. acceleration = 0\)

e.g. Initial position = 0 meters, a = 0, initial velocity +4 m/s, time = 3 seconds

D = 4 m/s \* 3 seconds = 12 meters

1. We can combine the first two equations by solving for time in equation 1 and substituting this expression for t into equation 2. You should do these algebra steps and let me know if you can't get
...
**\*g** is the symbol for the **acceleration due to gravity** Near the earth's surface the value of g is about 9.80 m/s2.
...
Note: Some people have a misconception that the value of the free-fall acceleration depends on the weight of the object. In reality, all objects near the earth have a free-fall acceleration of g \(9.8 m/s2\) if air resistance is ignored. We will prove this when we discuss force and Newton's laws.
