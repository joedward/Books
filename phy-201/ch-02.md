...
1. You should understand how equation 2.28 is found . Equation 2.28 lets us calculate the final position if we know the initial position, the average velocity and the time. It turns out that the average velocity can be found by adding the initial and final velocities and then dividing by 2.
...
1. near page 57 in pdf We can combine the first two equations by substituting for V in equation 2 with the expression for V from equation 1. This will generate an equation that does not contain the final velocity. Start with equation 2 X = X
